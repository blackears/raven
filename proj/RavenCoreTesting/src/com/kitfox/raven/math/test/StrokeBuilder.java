...
{
    ArrayList<PenPoint> points = new ArrayList<PenPoint>();
    ...
    static final double MAX_ERROR = 1000;
    static boolean useWeightedStrokes = true;
    ...
    public void addPoint(PenPoint pt)
    {
        if (!points.isEmpty())
        {
            PenPoint ptLast = points.get(points.size() - 1);
...
//                System.err.println();
        ...
        //!!!!!!!!!!!!!!
        if (useWeightedStrokes && !segments.isEmpty())
        {
//            OutlinerPath path = new OutlinerPath(10000);
//            for (int i = 0; i < segments.size(); ++i)
//            {
//                FitCurveRecord curSeg = segments.get(i);
//
//                if (i == 0)
//                {
//                    path.moveTo(curSeg.seg.getA0x(), curSeg.seg.getA0y(),
//                            (float)curSeg.weight0);
//                }
//
//                path.cubicTo(curSeg.seg.getA1x(), curSeg.seg.getA1y(),
//                        curSeg.seg.getA2x(), curSeg.seg.getA2y(),
//                        curSeg.seg.getA3x(), curSeg.seg.getA3y(),
//                        (float)curSeg.weight1);
//            }
//
//            Path2D pathAwt = path.getOutlinePath(Cap.ROUND, Join.ROUND, 500);

            Path2D pathAwt = outlinerPath.getOutlinePath(Cap.ROUND, Join.ROUND, 500);
//            Path2D pathAwt = outlinerPath.getOutlinePath(Cap.BUTT, Join.BEVEL, 500);

            displayPath = new PathCurve(pathAwt);
        }
        //!!!!!!!!!!!!!!
    }
