...
public class StyleStrokeWidthLoader extends StyleElementLoader
{

    public StyleStrokeWidthLoader()
    {
        super("stroke-width", StyleKey.STROKE_WIDTH);
    }


    @Override
    public RaLength parse(String value, RabbitDocument builder)
    {
        AttributeParser parser = new AttributeParser(new StringReader(value));
        try {
            return parser.Length();
        } catch (ParseException ex) {
            Logger.getLogger(StyleFontSizeLoader.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }
