...
public class ActionGetURL2 extends ActionRecord
{
    public static class Loader extends ActionRecordLoader
    {
        ...
        {
            int method = (int)in.getUB(2);
            in.getUB(4);
            boolean loadTarget = in.getUB(1) != 0;
            boolean loadVariables = in.getUB(1) != 0;
            in.flushToByteBoundary();

            return new ActionGetURL2(SendVarsMethod.values()[method], loadTarget, loadVariables);
        }
    }

    public static final int CODE = 0x9A;

    static public enum SendVarsMethod { NONE, GET, POST };
    SendVarsMethod method;
    boolean loadTarget;
    boolean loadVariables;

    public ActionGetURL2(SendVarsMethod method, boolean loadTarget, boolean loadVariables) {
        this.method = method;
