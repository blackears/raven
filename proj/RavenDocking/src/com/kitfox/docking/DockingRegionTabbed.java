        ...
        implements DockingChild
{
    ...
    HashMap<DockingContent, TabLayout> tabs = new HashMap<DockingContent, TabLayout>();
...
//        dockParent.floatWindow(content, path);
    ...
    public void restore(DockingContent content, DockingPathRecord subpath)
    {
        if (subpath instanceof PathRecordTabbed)
        {
            ...
            int index = Math.min( rec.getIndex(), getTabCount());
    ...
    {
        for (DockingContent cont: new ArrayList<DockingContent>(tabs.keySet()))
        {
            removeTab(cont);
        }
    }
